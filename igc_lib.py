...
class Thermal:
    ...
    def lon_velocity(self):
        """Returns average longitudinal velocity in the thermal, m/s."""
        return geo.earth_distance(self.exit_fix.lat, self.enter_fix.lon, self.exit_fix.lat, self.exit_fix.lon)*1000/self.time_change()

    def lat_velocity(self):
        """Returns average longitudinal velocity in the thermal, m/s."""
        return geo.earth_distance(self.enter_fix.lat, self.enter_fix.lon, self.exit_fix.lat, self.enter_fix.lon)*1000/self.time_change()
    ...
    def __str__(self):
        ...
        return ("Thermal(vertical_velocity=%.2f m/s, duration=%dm %ds, lon_velocity = %.2f m/s, lat_velocity = %.2f m/s )" %
                (self.vertical_velocity(), hms.minutes, hms.seconds, self.lon_velocity(), self.lat_velocity()))
